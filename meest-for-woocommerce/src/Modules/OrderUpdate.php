...
class OrderUpdate
{
    ...
    public function checkoutUpdateOrderMeta($id)
    {
        if (self::isMeestShipping()) {
            ...
            if (defined('WC_VERSION') && defined('WC_VERSION') && version_compare(WC_VERSION, '4.3.0', '<')) {
    ...
    private function updateOrderData($id, $address, $type)
    {
        if (!empty($address['country'])) {
            update_post_meta($id, "_{$type}_country", $address['country']);
        }
        ...
        if (!empty($address['address_2'])) {
            update_post_meta($id, "_{$type}_address_2", $address['address_2']);
        }
    }
    ...
    public function thankyou($orderId)
    {
        ...
        if ($order->has_shipping_method(MEEST_PLUGIN_NAME)) {
            ...
            $receiverData = $orderShipping->get_meta('receiver');
            // Десериализуем данные, если они были сериализованы
            if (is_string($receiverData)) {
                $receiverData = maybe_unserialize($receiverData);
            }

            echo View::part('views/parts/address_details', [
                'address' => $receiverData
            ]);
        }
    }
