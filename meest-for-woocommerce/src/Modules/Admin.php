...
class Admin implements Module
{
    ...
    public function init()
    {
        if (is_admin()) {
            ...
            // Регистрируем хуки для колонок заказов
            // HPOS (новая система хранения заказов WooCommerce)
            add_filter('manage_woocommerce_page_wc-orders_columns', [$this, 'shopOrderColumns'], 10);
            add_action('woocommerce_shop_order_list_table_custom_column', [$this, 'pageWcOrdersCustomColumns'], 10, 2);
            
            // Legacy (старая система постов)
            add_filter('manage_edit-shop_order_columns', [$this, 'shopOrderColumns'], 10);
            add_action('manage_shop_order_posts_custom_column', [$this, 'shopOrderCustomColumn'], 10, 1);
    ...
    {
        $columns['meest_shipping_method'] = __('Shipping Method', MEEST_PLUGIN_DOMAIN);
    ...
    public function shopOrderCustomColumn($column)
    {
        ...
        $this->pageWcOrdersCustomColumns($column, $order);
    }
     ...
     * @since 1.1.0
     */
    public function pageWcOrdersCustomColumns($column, $order)
    {
        // Колонка с методом доставки - показываем для всех заказов
        if ($column === 'meest_shipping_method') {
            $shipping_methods = $order->get_shipping_methods();
            if (!empty($shipping_methods)) {
                foreach ($shipping_methods as $shipping_method) {
                    // Проверяем, является ли это методом Meest
                    if (strpos($shipping_method->get_method_id(), MEEST_PLUGIN_NAME) === 0) {
                        // Выводим название метода доставки с иконкой
                        echo '<div style="display: flex; align-items: center; gap: 5px;">';
                        echo '<img src="'.MEEST_PLUGIN_URL.'public/img/icon.png'.'" height="16px" style="vertical-align: middle;">';
                        echo '<span>' . esc_html($shipping_method->get_name()) . '</span>';
                        echo '</div>';
                        break;
                    }
                }
        ...
        // Остальные колонки только для заказов с Meest доставкой
        if ($order->has_shipping_method(MEEST_PLUGIN_NAME)) {
            $parcel = Parcel::find($order->get_id(), 'order_id');

            if ($column === 'created_invoice') {
                if ($parcel !== null) {
                    // Если накладная создана - цветная иконка со ссылкой на декларацию
                    $declarationUrl = meest_init('Api')->getUrlDeclaration($parcel->parcel_id);
                    echo '<a target="_blank" href="'.$declarationUrl.'" title="'.__('View invoice', MEEST_PLUGIN_DOMAIN).'"><img src="'.MEEST_PLUGIN_URL.'public/img/icon_big.png'.'" height="20px"></a>';
                } else {
                    // Если накладной нет - серая иконка со ссылкой на создание
                    echo '<a href="admin.php?page=meest_parcel&action=create&post='.$order->get_id().'" title="'.__('Create parcel', MEEST_PLUGIN_DOMAIN).'"><img src="'.MEEST_PLUGIN_URL.'public/img/icon_big.png'.'" height="20px" style="filter: grayscale(100%)"></a>';
                }
            ...
            } elseif ($column === 'wcus_ttn_actions') {
                echo '<span style="display: none;">';
            }
        }
    }
